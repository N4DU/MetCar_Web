...
(function () {
  ...
  /* Pedidos del cliente (historial) — mapa email → [pedidos] */
  function allOrders() { try { return JSON.parse(localStorage.getItem('mc_orders') || '{}'); } catch (e) { return {}; } }
  function saveAllOrders(o) { localStorage.setItem('mc_orders', JSON.stringify(o)); }

  /* Estados del pedido (Flujo D del documento de especificaciones) */
  var ORDER_STATES = [
  ...
  var MCAuth = {
    ...
    sendQuote: async function (items, nota) {
      ...
      // Guardar el pedido en el historial del cliente (para "Mis pedidos")
      try {
        var order = {
          ref: ref,
          createdAt: new Date().toISOString(),
          estadoIdx: (u.estado === 'validado') ? 2 : 1,
          nota: (nota || '').trim(),
          items: (items || []).map(function (i) {
            return {
              code: i.code, name: i.p.name, qty: i.qty,
              specs: i.p.specs || [], configured: !!i.p.configured,
              detail: i.p.detail || [], stock: i.p.stock !== false, cat: i.p.cat || ''
            };
          })
        };
        var all = allOrders();
        (all[u.email] = all[u.email] || []).unshift(order);
        saveAllOrders(all);
      } catch (e) {}
      return { ref: ref, result: r, estado: u.estado };
    },

    /* Estados del pedido (expuesto para la página Mi cuenta) */
    STATES: ORDER_STATES,

    /* Pedidos del cliente logueado, más nuevos primero */
    orders: function () {
      var u = MCAuth.current(); if (!u) return [];
      return (allOrders()[u.email] || []).slice();
    },

    /* Actualizar un pedido del cliente (estado, comprobante, etc.) */
    updateOrder: function (ref, patch) {
      var u = MCAuth.current(); if (!u) return null;
      var all = allOrders();
      var arr = all[u.email] || [];
      var o = arr.find(function (x) { return x.ref === ref; });
      if (!o) return null;
      Object.assign(o, patch);
      saveAllOrders(all); notify();
      return o;
    },

    /* Editar datos del perfil del cliente logueado */
    updateProfile: function (d) {
      var s = session(); if (!s) throw new Error('No hay sesión activa.');
      var all = users();
      var u = all.find(function (x) { return x.email === s.email; });
      if (!u) throw new Error('No se encontró la cuenta.');
      if (d.nombre != null && d.nombre.trim()) u.nombre = d.nombre.trim();
      if (d.tel != null) u.tel = String(d.tel).trim();
      if (d.tipo) u.tipo = d.tipo;
      if (d.dir) u.dir = Object.assign(u.dir || {}, d.dir);
      saveUsers(all); notify();
      return u;
    }
  };
  ...
  function buildAcct(orig) {
    ...
    menu.innerHTML =
      ...
      + '<div class="mc-acct-links">'
        + '<button class="mc-acct-li" data-go="carrito.html"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/><path d="M3 6h18"/><path d="M16 10a4 4 0 0 1-8 0"/></svg>Carrito</button>'
        + '<button class="mc-acct-li" data-go="mi-cuenta.html"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/><path d="M9 13l2 2 4-4"/></svg>Historial de pedidos</button>'
        + '<button class="mc-acct-li" data-go="mi-cuenta.html?tab=perfil"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>Mi perfil</button>'
