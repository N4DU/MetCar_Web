...
(function () {
  ...
  /* ── Carpeta de fotos por categoría ──
     Convención: img/catalogo/<CARPETA>/<CODIGO>.webp (o .png)
     Para darle foto a un producto: subí el archivo nombrado con su CÓDIGO
     (ej. MC-TH-209.webp) a la carpeta de su categoría. Se toma solo. ── */
  var CAT_FOLDER = {
    mangueras:  'Mangueras_Hidraulicas',
    terminales: 'Terminales_Hidraulicos',
    discos:     'Discos_de_Corte',
    torno:      'Herramientas_de_Torno',
    gatos:      'Gatos_Hidraulicos',
    prensas:    'Prensas_y_Accesorios',
  };
  ...
  var PRODUCTS = [
    ...
    ...(((window.MC_TERMINALES && window.MC_TERMINALES.items) || []).map(function (t) {
      return {
        ...
        stock: t.stock, tipo: 'simple', img: t.img || null
      };
    })),
    // ── Discos de corte
  ...
  window.MC_CATALOG = {
    ...
    /* Ruta base de la foto (sin extensión) según código + categoría.
       El render prueba .webp y si no existe .png; si no hay foto, usa el ícono. */
    photoBase: function (p) {
      var f = CAT_FOLDER[p.cat];
      return f ? ('img/catalogo/' + f + '/' + p.code) : null;
    },
  };
})();
